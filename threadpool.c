...
typedef struct threadPool
{
	...
	pthread_mutex_t mutexBusy;	//忙碌的线程个数互斥锁，抢到这把锁的线程，防止在任务队列访问临界区的忙碌线程个数
...
void* worker(void* arg)
{
	...
	while (1)
	{
		...
		//判断当前队列是否为空
		while (pool->queueSize == 0 && pool->shutdown != 1)	//当消费者消费后，该线程队列中这个位置就为空了，继续往下就会阻塞在下一语句了，这就是为什么要写while
		{
